...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **`User`** | `users` | `id` (bigint) | `user_code`, `name`, `username`, `email`, `password`, `registered_by`, `avatar`, `phone`, `no_rekening`, `bank`, `address` | PK Auto-increment, `user_code` default '0' | - `masyarakat()`: hasOne `Masyarakat`<br>- `registeredBy()`: belongsTo `User`<br>- `createdBankSampahUsers()`: hasMany `BankSampahUser`<br>- `tabungans()`: hasMany `Tabungan`<br>- `orders()`: hasMany `Order` |

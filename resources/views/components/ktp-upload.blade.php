...
<div id="{{ $uid }}" class="w-full" data-ktp-upload>
    <label class="label">
        <span class="label-text text-sm font-medium text-stone-700">{{ $label }}</span>
    </label>
    ...
    >
        ...
        <div data-empty-state class="flex flex-col items-center gap-2">
            ...
            <p class="text-sm text-base-content/70">
                Klik untuk upload {{ Str::lower($label) }}
            </p>
            <p class="text-xs text-base-content/40">
                Maks {{ $maxSizeMb }}MB &middot; JPG / PNG / WebP
            </p>
        </div>

        {{-- State terisi: preview + nama file (disembunyikan default via class hidden) --}}
    ...
    {{-- Pesan error validasi server-side Laravel --}}
    @error($name)

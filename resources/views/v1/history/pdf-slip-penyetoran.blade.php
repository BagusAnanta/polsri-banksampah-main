    ...
    <table border="0" style="border: 2px solid black;border-bottom: none;">
        <tr>
            <td>No. Rekening <span style="margin-left: 100px">:
                    {{ Auth::user()->no_rekening . ' ' . '(' . Auth::user()->bank . ')' }}</span></td>
            <td>Periode : <span>{{ $months[$month].' '.$year }}</span></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td style="width: 500px">
                ...
                <div style="width: 20px;height: 15px;border: 1px solid black;float: left;">
                    {{-- <img style="float: left;margin-top: -10px" src="assets/icons/ceklis.png" width="25"
                        height="25" alt=""> --}}
                </div>
                <span style="float: left;margin-left: 5px;margin-right: 50px">
                    RUPIAH
        ...
        <tr>
            <td>No. Tlp <span style="margin-left: 139px">: {{ Auth::user()->phone }}</span><br><br></td>
            <td>Terbilang : {{ Str::ucfirst($totalHargaTerbilang) }}<br><br></td>
        </tr>
        <tr>
            ...
            @else
                @foreach ($items as $item)
                    <tr>
                        ...
                        <td style="border: 2px solid black">{{ $item->jenisSampah->nama }}</td>
                        <td style="border: 2px solid black">
                            {{ 'Rp. ' . number_format($item->jenisSampah->harga, 0, ',', ',') }}
                        </td>
                        <td style="border: 2px solid black">{{ $item->qty }}</td>
                        <td style="border: 2px solid black">
                            {{ 'Rp. ' . number_format($item->jenisSampah->harga * $item->qty, 0, ',', ',') }}
                        </td>
                    </tr>
                @endforeach
                <tr>
                    ...
                    <td style="border: 2px solid black;font-weight: 700">
                        {{ 'Rp. ' . number_format($totalHarga, 0, ',', ',') }}</td>
                </tr>
            @endif

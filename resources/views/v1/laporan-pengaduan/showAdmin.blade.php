    ...
    <header class="header header-fixed">
        <div class="header-content">
            ...
            <div class="right-content d-flex align-items-center gap-4">
                ...
                {{-- <a href="#" onclick="modalDeleteMobile('jenis_sampahs/' + {{ $laporan->id }}, '/jenis_sampahs/')" class="btn shadow sharp"><i
                   class="fa fa-trash"></i></a> --}}
                <form action="{{ route('laporan-pengaduans.destroyAdmin', $laporan->id) }}" method="post">
                    @csrf
                    @method('DELETE')
                ...
                {{-- @endif --}}
                {{-- <a href="{{ route('data-products.edit', $laporan->id) }}">
            ...
            <ul>
                <li>
                    <div class="dz-content">
                        ...
                        <h6 class="title">{{ $laporan->user->user_code }}</h6>
                    </div>
                </li>
                <li>
                    <div class="dz-content">
                        <p class="sub-title">Nama Nasabah</p>
                        <h6 class="title">{{ $laporan->user->name }}</h6>
                    </div>
                </li>
                <li>
                    <div class="dz-content">
                        <p class="sub-title">ID Box</p>
                        <h6 class="title">{{ $laporan->boxSampah->id_box }}</h6>
                    </div>
                </li>
                <li>
                    <div class="dz-content">
                        <p class="sub-title">Catatan</p>
                        <h6 class="title">{{ $laporan->catatan }}</h6>
                    </div>
                </li>
            </ul>
        </div>

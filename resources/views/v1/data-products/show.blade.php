         ...
         <div class="header-content">
             ...
             <div class="right-content d-flex align-items-center gap-4">
                 @if (Auth::user()->getRoleNames()[0] == 'Admin')
                     {{-- <a href="#" onclick="modalDeleteMobile('jenis_sampahs/' + {{ $product->id }}, '/jenis_sampahs/')" class="btn shadow sharp"><i
                    class="fa fa-trash"></i></a> --}}
                     <form action="{{ route('data-products.destroy', $product->id) }}" method="post">
                         @csrf
                         @method('DELETE')
                 ...
                 @endif
                 <a href="{{ route('data-products.edit', $product->id) }}">
             ...
             <ul>
                 <li>
                     <div class="dz-content">
                         ...
                         <h6 class="title">{{ ucfirst($product->name) }}</h6>
                     </div>
                 </li>
                 <li>
                     <div class="dz-content">
                         <p class="sub-title">Deskripsi</p>
                         <h6 class="title">{{ $product->description }}</h6>
                     </div>
                 </li>
                 <li>
                     <div class="dz-content">
                         <p class="sub-title">Harga</p>
                         <h6 class="title">{{ 'Rp. ' . number_format($product->price, 0, ',', ',') }}</h6>
                     </div>
                 </li>
                 <li>
                     <div class="dz-content">

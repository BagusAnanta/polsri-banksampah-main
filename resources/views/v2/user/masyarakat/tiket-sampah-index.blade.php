<x-layouts.app title="Tiket Setor Sampah" :subtitle="($tikets ?? collect())->count() . ' tiket tercatat'">
    ...
    <div class="space-y-3">
            ...
            <div class="overflow-hidden rounded-xl bg-base-200/50 transition hover:bg-base-200">
                <div class="p-3 sm:p-4">
                    <div class="mb-3 flex items-center justify-between">
                        <div>
                            <p class="text-xs font-medium text-base-content sm:text-sm">Tiket #{{ $tiket->tiketsampah_inc }}</p>
                    ...
                    <div class="mb-3 flex flex-wrap gap-2 text-xs">
                        ...
                        @endif
                        <span class="rounded-full bg-base-300/50 px-2.5 py-0.5">{{ number_format($tiket->berat_sampah ?? 0) }}g est.</span>
                    </div>

                    @if(strtolower($tiket->status) === 'menunggu')
                        <div class="flex flex-col gap-2 sm:flex-row">
                            <button class="btn btn-outline btn-sm flex-1 rounded-lg text-xs" 
                                    onclick="document.getElementById('qr_modal_{{ $tiket->tiketsampah_id }}').showModal()">
                                📱 Tampilkan QR
                            </button>
                            <form method="POST" action="{{ route('tiket-sampah.cancel', $tiket->tiketsampah_id) }}" class="flex-1">
                                @csrf
                                @method('PUT')
                        ...
                        <dialog id="qr_modal_{{ $tiket->tiketsampah_id }}" class="modal">
                            <div class="modal-box w-full max-w-xs">
                                <h3 class="text-lg font-bold">QR Code Tiket</h3>
                                <div class="py-4 text-center">
                                    <div id="qr_{{ $tiket->tiketsampah_id }}" class="mx-auto"></div>
                                    <p class="mt-2 text-sm text-base-content/60">Tiket #{{ $tiket->tiketsampah_inc }}</p>
                                </div>
                        ...
                        <script>
                            new QRCode(document.getElementById("qr_{{ $tiket->tiketsampah_id }}"), {
                        ...
                        @endpush
                    @else
                        <a href="{{ route('tiket-sampah.show', $tiket->tiketsampah_id) }}" class="btn btn-outline btn-sm w-full rounded-lg text-xs">
                            Lihat Detail
                        </a>
                    @endif
                </div>
        ...
        @empty
            <div class="rounded-xl bg-base-200/30 p-8 text-center">
                ...
                <a href="{{ route('tiket-sampah.create') }}" class="btn btn-primary btn-sm mt-3 rounded-full">
                    Buat Tiket Pertama
                </a>
            </div>
        @endforelse

            ...
            <div class="card-body gap-4 p-6">
                ...
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-olive-800">#{{ $tiket->tiketsampah_inc }}</p>
                ...
                <div class="pt-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <p class="text-xs text-base-content/50">Estimasi Berat</p>
                            <p class="text-lg font-semibold">{{ number_format($tiket->berat_sampah) }} gram</p>
                        </div>
                        <div>
                            <p class="text-xs text-base-content/50">Berat Aktual</p>
                            <p class="text-lg font-semibold">
                                @if($tiket->berat_sampah_actual)
                                    {{ number_format($tiket->berat_sampah_actual) }} gram
                                @else
                                    <span class="text-base-content/40">Belum divalidasi</span>
                                @endif
                            </p>
                        </div>
                    ...
                    @if(strtolower($tiket->status) === 'selesai')
                        <div class="mt-3 flex items-center gap-2 rounded-xl bg-green-50 p-3">
                            ...
                            <p class="text-sm text-green-700">
                                Poin diperoleh:
                                <span class="font-semibold">+{{ number_format($tiket->poin) }}</span>
                            </p>
                        </div>
                    @endif
                </div>
                ...
                @if(strtolower($tiket->status) === 'menunggu')
                    <div class="flex gap-3 pt-4">
                        <button
                            ...
                            onclick="document.getElementById('qr-modal-{{ $tiket->tiketsampah_id }}').showModal()"
                        >
                            Lihat QR
                        </button>
                    ...
                    <dialog id="qr-modal-{{ $tiket->tiketsampah_id }}" class="modal">
                        <div class="modal-box max-w-xs text-center">
                            <h3 class="font-semibold">QR Tiket #{{ $tiket->tiketsampah_inc }}</h3>

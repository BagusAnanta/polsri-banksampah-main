    ...
    <form action="{{ route('sa.edukasi.update', $artikel->artikel_id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-[#E1EFE3] rounded-xl p-6 border border-slate-200 space-y-4">
            ...
            <div class="form-control">
                <input type="text" name="judul_artikel" class="input input-bordered rounded-xl bg-[#E1EFE3] text-sm px-4 py-3 pr-12 text-stone-800 placeholder-[#5B6B63] w-full" value="{{ old('judul_artikel', $artikel->judul_artikel) }}" required>
                @error('judul_artikel')

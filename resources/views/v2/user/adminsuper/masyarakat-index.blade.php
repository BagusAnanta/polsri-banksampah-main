...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse($masyarakat as $m)
            <div class="bg-[#E4CEA5] rounded-xl p-5 border border-slate-200" data-verification="{{ $m->verification }}">
                ...
                <div class="mt-3 flex gap-2">
                    @if($m->verification === 'Menunggu')
                        <a href="{{ route('sa.masyarakat.review', $m->masyarakat_id) }}" class="btn rounded-full btn-sm flex-1 bg-[#A3A85C] border-[#A3A85C] hover:bg-[#8B904A]">Tinjau</a>
                    @else
                        <a href="{{ route('sa.masyarakat.show', $m->masyarakat_id) }}" class="btn rounded-full bg-[#F2DFBC] border-[#A48B5C] hover:bg-[#E4CEA5] btn-sm flex-1">Lihat</a>
                    @endif
                </div>
            </div>
        @empty

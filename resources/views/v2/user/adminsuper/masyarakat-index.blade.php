...
<x-layouts.app title="Daftar Masyarakat" subtitle="Semua pengguna masyarakat yang terdaftar">
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse($masyarakat as $m)
            <div class="bg-[#E1EFE3] rounded-xl p-5 border border-slate-200" data-verification="{{ $m->verification }}">
                ...
                <div class="mt-3 flex gap-2">
                    @if($m->verification === 'Menunggu')
                        <a href="{{ route('sa.masyarakat.review', $m->masyarakat_id) }}" class="btn rounded-full btn-sm flex-1 bg-[#16A34A] border-[#16A34A] hover:bg-[#16A34A]">Tinjau</a>
                    @else
                        <a href="{{ route('sa.masyarakat.show', $m->masyarakat_id) }}" class="btn rounded-full bg-[#F3F8F4] border-[#5B6B63] hover:bg-[#E1EFE3] btn-sm flex-1">Lihat</a>
                    @endif
                </div>
            </div>
        @empty
